...
void ofApp::setupLighting(){
    // Global ambient so shadowed areas aren't crushed
    ofSetGlobalAmbientColor(ofColor(90, 90, 100));

    // Key light (directional, warm-ish) from above-left-front
    keyLight.setDirectional();
    keyLight.setDiffuseColor(ofFloatColor(0.95f, 0.95f, 0.98f));
    keyLight.setSpecularColor(ofFloatColor(1.0f, 1.0f, 1.0f));
    keyLight.setPosition(700, 800, 900);
    keyLight.lookAt(glm::vec3(0,0,0), glm::vec3(0,0,1));

    // Fill (cooler) from the opposite side to lift shadows
    fillLight.setDirectional();
    fillLight.setDiffuseColor(ofFloatColor(0.45f, 0.50f, 0.60f));
    fillLight.setSpecularColor(ofFloatColor(0.20f, 0.22f, 0.25f));
    fillLight.setPosition(-900, -700, 500);
    fillLight.lookAt(glm::vec3(0,0,0), glm::vec3(0,0,1));

    // Material: let specular pop; ambient/diffuse will come from vertex color (see draw)
    mat.setShininess(64);                                  // tighter highlight
    mat.setSpecularColor(ofFloatColor(1.0f, 1.0f, 1.0f));  // bright spec
    mat.setAmbientColor(ofFloatColor(0.20f, 0.20f, 0.22f));
    // We won't set mat diffuse because we'll drive ambient+diffuse from vertex colors.
}
...
void ofApp::update(){
    ...
    // ---- Hand → continuous drawing (no pinch) ----
    if(handControlEnabled && !bLeftDown){ // never interfere with real LMB
        auto sOpt = hand.latest();
        if(sOpt && sOpt->hasHand){
            float nx = sOpt->normX;
            float ny = sOpt->normY;

            // Exponential smoothing in normalized space
            if(handHadPrev){
                nx = ofLerp(handNormPrev.x, nx, handSmoothAlpha);
                ny = ofLerp(handNormPrev.y, ny, handSmoothAlpha);
            }
            handNormPrev = {nx, ny};
            handHadPrev = true;

            // Axis correction before mapping to pixels
            float rx = nx, ry = ny;
            if(handSwapXY) std::swap(rx, ry);
            if(handFlipX)  rx = 1.0f - rx;
            if(handFlipY)  ry = 1.0f - ry;

            // Vision is bottom-left; our UI is top-left, so invert Y once here:
            int sx = int(std::round(rx * ofGetWidth()));
            int sy = int(std::round((1.0f - ry) * ofGetHeight()));

            float now = ofGetElapsedTimef();

            // Start stroke if we aren't already in one
            if(!handCurrentlyDrawing){
                handCurrentlyDrawing = true;
                ...
                glm::vec3 w = screenToXYPlane(sx, sy);
                strokes[cur].addVertex(glm::vec3(w.x, w.y, lineZLift));
                strokeVels[cur].push_back(0.f);
                strokeTimes[cur].push_back(now);

                injectImpulse(glm::vec2(w.x, w.y), 0.20f); // mild kick to begin
            } else {
                // Continue stroke with spacing & velocity-mapped impulse
                glm::vec3 w = screenToXYPlane(sx, sy);

                const auto &verts = strokes[cur].getVertices();
                const float spacing = 3.5f; // tighter spacing for smoother lines
                bool shouldAdd = verts.empty()
                               || glm::distance(verts.back(), glm::vec3(w.x, w.y, lineZLift)) >= spacing;

                if(shouldAdd){
                    float dt = std::max(1e-4f, now - prevTime);
                    float vpx = ofPoint(sx, sy).distance(prevMouse) / dt; // pixels/sec

                    strokes[cur].addVertex(glm::vec3(w.x, w.y, lineZLift));
                    strokeVels[cur].push_back(vpx);
                    strokeTimes[cur].push_back(now);

                    // Translate screen velocity to wave strength (soft-clamped)
                    float vClamped = ofClamp(vpx, 0.f, velClamp); // you already have velClamp
                    float strength = ofMap(vClamped, 0.f, velClamp, 0.10f, 1.10f, true);
            ...
            // No hand visible — gracefully end the virtual stroke
            if(handCurrentlyDrawing){
                if(cur >= 0 && strokes[cur].size() < 2){
                    strokes.pop_back(); strokeVels.pop_back(); strokeTimes.pop_back();
                }
                cur = -1;
                handCurrentlyDrawing = false;
            }
            handHadPrev = false;
        }
    }
...
void ofApp::draw(){
    ...
    ofDrawBitmapStringHighlight(
        ...
        12, ofGetHeight()-18);
    ofDrawBitmapStringHighlight(
        std::string("Hand Control [H]: ") + (handControlEnabled ? "ON (pinch to draw)" : "OFF"),
        12, ofGetHeight()-36);
    
    if(handControlEnabled){
        auto sOpt = hand.latest();
        if(sOpt && sOpt->hasHand){
            float nx = sOpt->normX, ny = sOpt->normY;

            // mirror debug smoothing to match feel (optional)
            if(handHadPrev){
                nx = ofLerp(handNormPrev.x, nx, handSmoothAlpha);
                ny = ofLerp(handNormPrev.y, ny, handSmoothAlpha);
            }
            ...
            ofDrawCircle(sx, sy, 14);
            ofFill();
            ofDrawCircle(sx, sy, 4);
            ofPopStyle();
        }
    }
}
...
void ofApp::drawWaveMeshLit(){
    ...
    // Use material for specular/shininess, but drive ambient+diffuse from vertex colors.
    mat.begin();
    glEnable(GL_COLOR_MATERIAL);
    glColorMaterial(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE);

    // IMPORTANT: do not override with a global color here
    waveMesh.draw();          // triangles with valid normals
...
void ofApp::buildWaveMesh(){
    ...
    for(int j=0; j<waveRes; ++j){
        ...
        for(int i=0; i<waveRes; ++i){
            ...
            waveMesh.addColor(ofFloatColor(1.0f, 1.0f, 1.0f, 1.f)); // grey fill
        }
    }
    // indices
