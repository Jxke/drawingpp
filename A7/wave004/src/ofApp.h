#pragma once
#include "ofMain.h"// Hand tracking (Vision/CoreML/CoreGraphics bridge)
...
//    std::vector<ofMesh> meshes;
    ...
    // Hand Tracking
    bool handControlEnabled = false;   // toggled with 'H'
    bool handPinchPrev = false;        // track pinch transitions
    bool handCurrentlyDrawing = false; // to avoid colliding with real mouse
    HandTracker hand;
    
    // Hand motion mapping & smoothing
    glm::vec2 handNormPrev = {0.5f, 0.5f};
    bool      handHadPrev  = false;
    float     handSmoothAlpha = 0.25f;   // 0..1 (higher = snappier, lower = smoother)

    // Axis correction for your current camera (fixes left→left, up→up)
    bool handSwapXY = true;   // swap X and Y from Vision
    bool handFlipX  = true;  // mirror horizontally
    bool handFlipY  = false;  // mirror vertically (after top-left mapping)
